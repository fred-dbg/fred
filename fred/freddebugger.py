...
#  and the remaining branch of "if" can be removed.
	...
	done = lambda: len(l_history) - n_min == 1 and l_history[-1].is_step()
	while not done():
	    ...
	    # Since we're stopped at a breakpoint, repeated 'next' commands
		...
		continue
	    # Should add something like a breakpoint here, or a check that
	    #   after repeated 'next' commands, stack is at least orig length
	    if l_history[-1].is_next():
	        (l_history, n_min) = \
		    self.NEW_binary_search_repeat_step(l_history, testIfTooFar)
...
# pdb.set_trace()
	...
	# Invariant:  TestIfTooFar() is always True at n_max and False at n_min
	    ...
	    if testIfTooFar():
                ...
                n_min = n_count
# DEBUGGING:
            if False and n_min > 2 and (n_min > 24 or n_max < 25):
...
#DEBUGGING:
    ...
    def NEW_binary_search_until(self, l_history, repeatCmd, testIfTooFar,
				itersToLive = -1):
        """Let init_history = initial l_history.
	...
	Returns (l_history, n_min)
	  ...
	  and l_history[0:len(init_history)] == init_history,
	  and l_history[len(init_history)+1:] consists entirely of repeatCmd.
